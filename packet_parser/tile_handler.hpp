...
class tile_hanlder {
 ...
 private:
  ...
  bool try_recover(tile_ *tile) {
    ...
    // walking past backward signals to find a forward one was tested and didn't help
    // in practice (the queue typically has only one future signal at recovery time).
