...
enum RTCP_ROLE { RECEIVER, SENDER };
...
struct rtcp_app_packet {
  ...
  rtcp_app_packet(const char* name, uint8_t subtype, uint32_t payload_len,
 ...
 *
 * \details If media_stream was created with RCE_RTCP flag, RTCP is enabled. RTCP periodically sends
 * compound RTCP packets. The bit rate of RTP session influences the reporting interval, but changing this
 * has not yet been implemented.
 *
 * The compound RTCP packet begins with either Sender Reports if we sent RTP packets recently or Receiver
 * Report if we didn't send RTP packets recently. Both of these report types include report blocks for all
 * the RTP sources we have received packets from during reporting period. The compound packets also always
 * have an SDES packet and calling send_sdes_packet()-function will modify the contents of this SDES packet.
 *
 ...
 *
 *
 * See <a href="https://www.rfc-editor.org/rfc/rfc3550#section-6" target="_blank">RFC 3550 section 6</a> for
 * more details.
 */
class rtcp {
 public:
  /// \cond DO_NOT_DOCUMENT
  rtcp(std::shared_ptr<uvgrtp::rtp> rtp, std::shared_ptr<std::atomic<std::uint32_t>> ssrc,
   ...
   *
   * Return RTP_OK on success and RTP_ERROR on error */
  rtp_error_t handle_incoming_packet(void* args, int rce_flags, uint8_t* buffer, size_t size,
                                     frame::rtp_frame** out);
  /// \endcond
  ...
  rtp_error_t set_network_addresses(std::string local_addr, std::string remote_addr, uint16_t local_port,
                                    uint16_t dst_port, bool ipv6);
   ...
   * \retval RTP_INVALID_VALUE If hook is nullptr
   */
  rtp_error_t install_sender_hook(
   ...
   * \retval RTP_INVALID_VALUE If hook is nullptr
   */
  rtp_error_t install_receiver_hook(
   ...
   *
   * \details Each time the RR/SR is sent, all APP sending hooks call their respective functions to get the
   * data
   *
  ...
  rtp_error_t recv_packet_handler_common(void* arg, int rce_flags, uint8_t* read_ptr, size_t size,
                                         frame::rtp_frame** out);
 ...
 private:
  ...
  uint32_t size_of_compound_packet(uint16_t reports, bool sr_packet, bool rr_packet, bool sdes_packet,
                                   uint32_t app_size, bool bye_packet) const;

  /* read the header values from rtcp packet */
  void read_rtcp_header(const uint8_t* buffer, size_t& read_ptr, uvgrtp::frame::rtcp_header& header);
  ...
  rtp_error_t handle_bye_packet(uint8_t* buffer, size_t& read_ptr, uvgrtp::frame::rtcp_header& header);
  ...
  double rtcp_interval(int members, int senders, double rtcp_bw, bool we_sent, double avg_rtcp_size,
                       bool red_min, bool randomisation);
  ...
  [[maybe_unused]] size_t tp_; /* the last time an RTCP packet was transmitted */
  [[maybe_unused]] size_t tc_; /* the current time */
  [[maybe_unused]] size_t tn_; /* the next scheduled transmission time of an RTCP packet */
  [[maybe_unused]] size_t
      pmembers_;   /* the estimated number of session members at the time tn was last recomputed */
  ...
  std::multimap<std::string,
                std::function<std::unique_ptr<uint8_t[]>(uint8_t& subtype, uint32_t& payload_len)>>
      outgoing_app_hooks_;
  bool hooked_app_;
