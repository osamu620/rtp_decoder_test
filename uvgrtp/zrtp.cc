...
void uvgrtp::zrtp::generate_zid() { uvgrtp::crypto::random::generate_random(session_.o_zid, 12); }
...
rtp_error_t uvgrtp::zrtp::validate_session() {
  ...
  /* Hello message */
  if (RTP_INVALID_VALUE
      == verify_hash((uint8_t *)hashes[2], (uint8_t *)session_.r_msg.hello.second, 81,
  ...
  if (session_.role == RESPONDER) {
    if (RTP_INVALID_VALUE
        == verify_hash((uint8_t *)hashes[1], (uint8_t *)session_.r_msg.commit.second,
                       session_.r_msg.commit.first - 8 - 4, session_.hash_ctx.r_mac[2])) {
  ...
  if (RTP_INVALID_VALUE
      == verify_hash((uint8_t *)hashes[0], (uint8_t *)session_.r_msg.dh.second,
                     session_.r_msg.dh.first - 8 - 4, session_.hash_ctx.r_mac[1])) {
...
rtp_error_t uvgrtp::zrtp::init(uint32_t ssrc, std::shared_ptr<uvgrtp::socket> socket, sockaddr_in &addr,
                               sockaddr_in6 &addr6, bool perform_dh, bool ipv6) {
  rtp_error_t ret = RTP_OK;
  ...
  if (perform_dh) {
    ...
    if (initialized_) {
      UVG_LOG_WARN(
          "ZRTP multistream mode not used. Please use RCE_ZRTP_MULTISTREAM_MODE flag "
...
rtp_error_t uvgrtp::zrtp::init_dhm(uint32_t ssrc, std::shared_ptr<uvgrtp::socket> socket, sockaddr_in &addr,
                                   sockaddr_in6 &addr6, bool ipv6) {
  rtp_error_t ret = RTP_OK;
  if (ipv6) {
    UVG_LOG_DEBUG("Starting ZRTP Diffie-Hellman negotiation with %s",
                  uvgrtp::socket::sockaddr_ip6_to_string(addr6).c_str());
  } else {
    UVG_LOG_DEBUG("Starting ZRTP Diffie-Hellman negotiation with %s",
                  uvgrtp::socket::sockaddr_to_string(addr).c_str());
  }
...
rtp_error_t uvgrtp::zrtp::get_srtp_keys(uint8_t *our_mkey, uint32_t okey_len, uint8_t *their_mkey,
                                        uint32_t tkey_len, uint8_t *our_msalt, uint32_t osalt_len,
                                        uint8_t *their_msalt, uint32_t tsalt_len) {
  if (!our_mkey || !their_mkey || !our_msalt || !their_msalt || !okey_len || !tkey_len || !osalt_len
      || !tsalt_len) {
    return RTP_INVALID_VALUE;
  }

  if (!initialized_) return RTP_NOT_INITIALIZED;
...
rtp_error_t uvgrtp::zrtp::packet_handler(void *args, int rce_flags, uint8_t *read_ptr, size_t size,
                                         frame::rtp_frame **out) {
  ...
  switch (msg->msgblock) {
    ...
    case ZRTP_MSG_HELLO_ACK: {
      ...
      if (!uvgrtp::crypto::crc32::verify_crc32(read_ptr, size - 4, ha_msg->crc)) return RTP_NOT_SUPPORTED;
      if (hello_ack_ != nullptr) {
    ...
    case ZRTP_MSG_COMMIT: {
      if (msg->length != 29 && msg->length != 25 && msg->length != 27)  // see rfc 6189 section 5.4
      {
      ...
      if (!uvgrtp::crypto::crc32::verify_crc32(read_ptr, size - 4, commit->crc)) return RTP_NOT_SUPPORTED;
    ...
    case ZRTP_MSG_DH_PART1: {
      ...
      if (!uvgrtp::crypto::crc32::verify_crc32(read_ptr, size - 4, dh->crc)) return RTP_NOT_SUPPORTED;
    ...
    case ZRTP_MSG_CONF2_ACK: {
      ...
      if (!uvgrtp::crypto::crc32::verify_crc32(read_ptr, size - 4, ca->crc)) return RTP_NOT_SUPPORTED;
...
void uvgrtp::zrtp::resetSession(uint32_t ssrc, std::shared_ptr<uvgrtp::socket> socket, sockaddr_in &addr,
                                sockaddr_in6 &addr6) {
