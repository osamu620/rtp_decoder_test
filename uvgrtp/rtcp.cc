...
[[maybe_unused]] const int MAX_PACKET   = 65536;

[[maybe_unused]] constexpr int ESTIMATED_MAX_RECEPTION_TIME_MS = 10;

const uint32_t MAX_SUPPORTED_PARTICIPANTS = 31;

uvgrtp::rtcp::rtcp(std::shared_ptr<uvgrtp::rtp> rtp, std::shared_ptr<std::atomic_uint> ssrc,
                   std::shared_ptr<std::atomic<uint32_t>> remote_ssrc, std::string cname,
...
uvgrtp::rtcp::rtcp(std::shared_ptr<uvgrtp::rtp> rtp, std::shared_ptr<std::atomic_uint> ssrc,
                   std::shared_ptr<std::atomic<uint32_t>> remote_ssrc, std::string cname,
                   std::shared_ptr<uvgrtp::socketfactory> sfp, std::shared_ptr<uvgrtp::srtcp> srtcp,
                   int rce_flags)
...
uvgrtp::rtcp_app_packet::~rtcp_app_packet() { delete[] name; }
...
void uvgrtp::rtcp::rtcp_runner(rtcp* rtcp) {
  ...
  while (rtcp->is_active()) {
    ...
    for (auto it = rtcp->ms_since_last_rep_.begin(); it != rtcp->ms_since_last_rep_.end(); ++it) {
      double timeout_interval_s = rtcp->rtcp_interval(int(rtcp->members_), 1, rtcp->rtcp_bandwidth_, true,
                                                      (double)rtcp->avg_rtcp_size_, false, false);
    ...
    double interval_s   = rtcp->rtcp_interval(int(rtcp->members_), 1, rtcp->rtcp_bandwidth_, true,
                                              (double)rtcp->avg_rtcp_size_, true, true);
...
rtp_error_t uvgrtp::rtcp::install_sender_hook(
    std::function<void(std::shared_ptr<uvgrtp::frame::rtcp_sender_report>)> sr_handler) {
  if (!sr_handler) {
    return RTP_INVALID_VALUE;
  }
...
rtp_error_t uvgrtp::rtcp::install_sender_hook(
    std::function<void(std::unique_ptr<uvgrtp::frame::rtcp_sender_report>)> sr_handler) {
  if (!sr_handler) {
    return RTP_INVALID_VALUE;
  }
...
rtp_error_t uvgrtp::rtcp::install_sdes_hook(
    std::function<void(std::shared_ptr<uvgrtp::frame::rtcp_sdes_packet>)> sdes_handler) {
  if (!sdes_handler) {
    return RTP_INVALID_VALUE;
  }
...
rtp_error_t uvgrtp::rtcp::install_sdes_hook(
    std::function<void(std::unique_ptr<uvgrtp::frame::rtcp_sdes_packet>)> sdes_handler) {
  if (!sdes_handler) {
    return RTP_INVALID_VALUE;
  }
...
rtp_error_t uvgrtp::rtcp::install_app_hook(
    std::function<void(std::shared_ptr<uvgrtp::frame::rtcp_app_packet>)> app_handler) {
  if (!app_handler) {
    return RTP_INVALID_VALUE;
  }
...
rtp_error_t uvgrtp::rtcp::install_app_hook(
    std::function<void(std::unique_ptr<uvgrtp::frame::rtcp_app_packet>)> app_handler) {
  if (!app_handler) {
    return RTP_INVALID_VALUE;
  }
...
void uvgrtp::rtcp::update_session_statistics(const uvgrtp::frame::rtp_frame* frame) {
  ...
  uint32_t arrival =
      participants_[frame->header.ssrc]->stats.initial_rtp
  ...
  participants_[frame->header.ssrc]->stats.jitter +=
      (1.f / 16.f) * ((double)trans_difference - participants_[frame->header.ssrc]->stats.jitter);
}
...
rtp_error_t uvgrtp::rtcp::recv_packet_handler_common(void* arg, int rce_flags, uint8_t* read_ptr,
                                                     size_t size, frame::rtp_frame** out) {
  (void)rce_flags;
  (void)size;
  (void)read_ptr;
...
rtp_error_t uvgrtp::rtcp::handle_incoming_packet(void* args, int rce_flags, uint8_t* buffer, size_t size,
                                                 frame::rtp_frame** out) {
  (void)args;
  (void)rce_flags;
  (void)out;
  ...
  if (size > RTCP_HEADER_SIZE + SSRC_CSRC_SIZE) {
    ...
    if (srtcp_
        && (ret = srtcp_->handle_rtcp_decryption(rce_flags_, sender_ssrc,
                                                 buffer + RTCP_HEADER_SIZE + SSRC_CSRC_SIZE, size))
               != RTP_OK) {
  ...
  while (remaining_size > 0) {
    ...
    if (remaining_size < RTCP_HEADER_SIZE) {
      UVG_LOG_ERROR("Didn't get enough data for an rtcp header. Packet # %i Got data: %lli", packets,
                    remaining_size);
      return RTP_INVALID_VALUE;
    }
    ...
    UVG_LOG_DEBUG("Handling packet # %i with size %li and remaining packet amount %li", packets,
                  size_of_rtcp_packet, remaining_size);
    ...
    if (header.pkt_type > uvgrtp::frame::RTCP_FT_APP || header.pkt_type < uvgrtp::frame::RTCP_FT_SR) {
...
void uvgrtp::rtcp::read_rtcp_header(const uint8_t* buffer, size_t& read_ptr,
                                    uvgrtp::frame::rtcp_header& header) {
  ...
  } else if (header.pkt_type == uvgrtp::frame::RTCP_FT_RTPFB
             || header.pkt_type == uvgrtp::frame::RTCP_FT_PSFB) {
...
void uvgrtp::rtcp::read_reports(const uint8_t* buffer, size_t& read_ptr, size_t packet_end, uint8_t count,
  ...
  for (int i = 0; i < count; ++i) {
    if (packet_end >= read_ptr + REPORT_BLOCK_SIZE) {
      ...
      UVG_LOG_WARN(
          "Received rtcp packet is smaller than the indicated number of reports!"
          "Read: %i/%i, Read ptr: %i, Packet End:",
          i, count, read_ptr, packet_end);
    }
  }
}
...
rtp_error_t uvgrtp::rtcp::handle_receiver_report_packet(uint8_t* buffer, size_t& read_ptr,
                                                        size_t packet_end,
                                                        uvgrtp::frame::rtcp_header& header) {
...
rtp_error_t uvgrtp::rtcp::handle_sender_report_packet(uint8_t* buffer, size_t& read_ptr, size_t packet_end,
  ...
  participants_[frame->ssrc]->stats.lsr =
      ((frame->sender_info.ntp_msw & 0xffff) << 16) | (frame->sender_info.ntp_lsw >> 16);
  participants_mutex_.unlock();
...
rtp_error_t uvgrtp::rtcp::handle_app_packet(uint8_t* packet, size_t& read_ptr, size_t packet_end,
                                            uvgrtp::frame::rtcp_header& header) {
...
rtp_error_t uvgrtp::rtcp::handle_fb_packet(uint8_t* packet, size_t& read_ptr, size_t packet_end,
                                           uvgrtp::frame::rtcp_header& header) {
  (void)packet_end;
  ...
  if (!is_participant(frame->sender_ssrc)) {
    UVG_LOG_INFO("Got an RTCP FB packet from a previously unknown participant SSRC %lu",
                 frame->sender_ssrc);
...
rtp_error_t uvgrtp::rtcp::send_rtcp_packet_to_participants(uint8_t* frame, uint32_t frame_size,
                                                           bool encrypt) {
  if (!frame) {
  ...
  if (encrypt && srtcp_
      && (ret = srtcp_->handle_rtcp_encryption(rce_flags_, rtcp_pkt_sent_count_, *ssrc_.get(), frame,
                                               frame_size))
             != RTP_OK) {
  ...
  if (rtcp_socket_ != nullptr) {
    std::lock_guard<std::mutex> prtcp_lock(participants_mutex_);
    if ((ret = rtcp_socket_->sendto(socket_address_, socket_address_ipv6_, frame, frame_size, 0))
        != RTP_OK) {
...
uint32_t uvgrtp::rtcp::size_of_compound_packet(uint16_t reports, bool sr_packet, bool rr_packet,
                                               bool sdes_packet, uint32_t app_size, bool bye_packet) const {
...
rtp_error_t uvgrtp::rtcp::generate_report() {
  ...
  if (hooked_app_) {
    ...
    for (auto& p : outgoing_app_hooks_) {
      ...
      if (p_len > 0 && sizeof(pload.get()) != 0) {
        std::shared_ptr<rtcp_app_packet> app_pkt =
            std::make_shared<rtcp_app_packet>(name.data(), subtype, p_len, std::move(pload));
        outgoing_apps_.push_back(app_pkt);
      }
    }
    app_packets_size = size_of_apps_from_hook(outgoing_apps_);
  }
  uint32_t compound_packet_size =
      size_of_compound_packet(reports, sr_packet, rr_packet, sdes_packet, app_packets_size, bye_packet);
  ...
  if (compound_packet_size == 0) {
    ...
    UVG_LOG_WARN(
        "Generate RTCP packet is too large %lli/%lli, reports should be circled, but not implemented!",
  ...
  if (sr_packet) {
    ...
    if (!construct_rtcp_header(frame, write_ptr, sender_report_size, reports, uvgrtp::frame::RTCP_FT_SR)
        ...
        || !construct_sender_info(frame, write_ptr, ntp_ts, reporting_rtp_ts, our_stats.sent_pkts,
                                  our_stats.sent_bytes)) {
  ...
  for (auto& p : participants_) {
    ...
    if (p.second->stats.received_rtp_packet) {
      ...
      construct_report_block(frame, write_ptr, p.first, uint8_t(fraction), dropped_packets,
                             p.second->stats.cycles, p.second->stats.max_seq,
                             (uint32_t)p.second->stats.jitter, p.second->stats.lsr, dlrs);
  ...
  if (app_packets_size != 0) {
    if (hooked_app_) {
      for (auto& pkt : outgoing_apps_) {
        if (!construct_app_block(frame, write_ptr, pkt->subtype & 0x1f, *ssrc_.get(), pkt->name,
                                 std::move(pkt->payload), pkt->payload_len)) {
      ...
      for (auto& app_name : app_packets_) {
        ...
        if (!app_name.second.empty()) {
          ...
          if (!construct_app_block(frame, write_ptr, next_packet.subtype & 0x1f, *ssrc_.get(),
                                   next_packet.name, std::move(next_packet.payload),
  ...
  UVG_LOG_DEBUG("Sending RTCP report compound packet, Total size: %lli", compound_packet_size);
...
rtp_error_t uvgrtp::rtcp::send_app_packet(const char* name, uint8_t subtype, uint32_t payload_len,
...
uint32_t uvgrtp::rtcp::get_rtcp_interval_ms() const { return interval_ms_.load(); }
...
std::shared_ptr<uvgrtp::socket> uvgrtp::rtcp::get_socket() const { return rtcp_socket_; }
...
double uvgrtp::rtcp::rtcp_interval(int members, int senders, double rtcp_bw, bool we_sent,
                                   double avg_rtcp_size, bool red_min, bool randomisation) {
...
void uvgrtp::rtcp::set_payload_size(size_t mtu_size) { mtu_size_ = mtu_size; }

void uvgrtp::rtcp::set_socket(std::shared_ptr<uvgrtp::socket> socket) { rtcp_socket_ = socket; }
