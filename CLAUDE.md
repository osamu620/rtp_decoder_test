...
- `BUILD_TESTS` (CMake option, OFF by default): builds `tests/prcl_crp_test`, a standalone harness that feeds a raw `.j2c`/`.jhc` through `parse_main_header` + `tile_handler::create()` and inspects the resulting CRP walk. `build/prcl_crp_test <stream> [dump|parse]` — `dump` prints the `c r p` progression order; `parse` additionally runs `flush()` over the whole codestream and asserts every precinct parses cleanly and fires in CRP order (exit 0 pass / 1 mismatch / 2 bad header / 3 unsupported). Used to validate PCRL/PRCL ordering against a reference decoder. Also builds `tests/incremental_delivery_test`, the contract test for `frame_handler`'s incremental delivery (chunk/abort callbacks + the `pull_data` gap parameter): `build/incremental_delivery_test <streamA.j2c> [streamB.j2c] [streamC.j2c]` runs 4 scenarios (clean ×2, mid-body gap, missed EOC, EOC-adjacent gap) and asserts byte-exact contiguous delivery, at-most-one abort, and no slab leaks. With `streamB` (**different image size** — the hatch scenario needs the structures to disagree) it adds the stream re-latch scenarios (geometry flip, mid-flip on a torn frame, parse-fail escape hatch), and with `streamC` (same geometry as A, different rate) the rate-only exemption. Run it on streams the parser supports (PCRL/custom-precinct family).

The parser's per-stream structures (`prec_`, `pband_`, `blk_`, tag-trees) live in a single 4 MB static arena in `packet_parser/utils.cpp` (`stackAlloc`). The arena is reset per frame via `tile_handler::restart` → `stackAlloc(0, 1)`. Allocations happen only inside `create()` (once per **latched stream**); pointers remain valid across the per-frame index reset because nothing else allocates from the arena. `create()` itself resets the arena to offset 0 first, so a stream re-latch (see below) rebuilds in place instead of accumulating dead structures.
...
State machine:
- On a Main packet (`MH >= 1`), release any stale held slabs (defensive: missed prior EOC), reset chain. If `MH >= 2` and `tile_hndr` not yet built, parse the J2K main header and call `tile_hndr.create()` to build the tile/component/resolution/precinct/codeblock tree. Subsequent frames verify the header's **geometry signature** (`j2k_header.hpp::geometry_signature` — FNV-1a 64 over the SIZ/COD/COC/QCD/QCC/DFS/ATK segments) against the latched stream's before re-using the tile structure; on a match they skip the full parse and take `start_SOD` from the fresh walk (benign header-length drift self-heals).
- **Stream re-latch** (mid-stream encoder re-dial recovery, 2026-07-15): a complete main header whose signature **differs** (e.g. an i5x3 ↔ i9x7 kernel flip — COD/ATK change; rate-only re-dials hash equal and are exempt) invalidates the latched stream and re-runs `parse_main_header` + `create()` on the new header, then fires the `set_stream_relatch_callback` (`kRelatchGeometry`) so consumers drop stream-scoped caches (e.g. a PID resync map). Escape hatch: `set_relatch_parse_fail_k(K)` (default 4) consecutive frames dead by PARSE failure (gap/loss frames are neutral) force a re-latch from the next clean main header (`kRelatchParseFail`) — this covers wedge shapes the signature can't see (e.g. MH==1 split-header senders). Before this existed, a mid-stream re-dial garbage-parsed every subsequent frame against the stale structure — a permanent parse-failure loop until relaunch (field wedge, 2026-07-14).
...
**Incremental (sub-frame) delivery** — for arrival-chasing consumers (e.g. a hardware decoder fed behind the RTP write cursor): `set_chunk_callback` fires for every byte range as it is appended (main header first at offset 0, then each body packet's J2K bytes; offsets strictly contiguous per frame; the pointer is valid only during the call), and `set_frame_abort_callback` fires at most once per frame the moment an in-flight frame becomes undecodable (`kAbortGap` / `kAbortParse` / `kAbortMissedEOC` / `kAbortSlabCap` / `kAbortDamagedEOC`). A frame terminates in EITHER frame-ready (all bytes delivered) or frame-abort. For immediate, deterministic loss detection the hook should pass `gap=true` into `pull_data` when it sees an RTP sequence discontinuity (the receiver delivers in order, so a seq skip IS a loss); without it, loss is only caught by the parser's resync checks. Both callbacks are composable with the per-precinct and frame-ready callbacks and cost one nullptr test per packet when unset.
...
- `j2k_header.cpp` — `parse_main_header` walks SOC/SIZ/CAP/COD/COC/QCD/DFS markers and returns the byte offset of the first SOD (**0 on failure**). The marker scan is **bounded**: any pre-SOD token that is not a `0xFFxx` marker (e.g. `get_word()` returning 0 past the chain end) or is `EOC` returns 0 instead of looping — a corrupt/truncated/desynced header must fail cleanly, not hang the worker, on lossy RTP transport. It also captures `Ccap^2` and emits a one-shot warning when a PRCL stream omits the Part-2 `EXTENDED_PROGRESSION` capability (Ccap2 bit 14); progression order is still taken from COD/COC, the authoritative field.
- `tile_handler.hpp` (header-only) — owns the parsed marker structures and the tile vector. `create()` builds the full tile→component→resolution→precinct→codeblock tree once per stream and populates `crp_idx_by_pid_` for recovery; it **returns `bool`** — `false` (and `frame_handler` fails the frame) when the stream is unbuildable, e.g. an unsupported progression order. `parse(PID)` advances `crp_idx` through precincts gated by the latest signaled resync byte. `flush()` drains remaining precincts at EOC. `restart(start_SOD)` clears the signal queue and per-frame parser state (it iterates `tiles.size()`, so a partially-built tile vector from a failed `create()` is safe).
- `j2k_packet.cpp` — `prepare_precinct_structure` (called once per stream from `create()`) builds the precinct walk `tile->crp` — the component/resolution/precinct visitation order the parser uses as packet identity. It supports **PCRL** (progression 3) and **PRCL** (progression 5, ISO/IEC 15444-2 extended progression); the two branches are identical except the inner two loops are transposed (PCRL `…for c: for r:`, PRCL `…for r: for c:`) — a pure local permutation within each spatial position-step. `parse_one_precinct` reads one precinct's packet header (SOP/EPH handling, tag-tree decode for inclusion/zero-bitplane, layer counts, codeblock lengths) and advances the codestream by the body size. Codeblock bodies that span chain chunks are materialized via `take_contiguous(length)` (heap-owned staging in `codestream::staging_` — must NOT use `stackAlloc`, that arena is reset every frame).
...
This is a **profile-specific** parser for the broadcast HTJ2K pipeline, not a general JPEG 2000 decoder. It assumes the stream conforms to the deployment's profile & levels document and deliberately does not handle out-of-profile inputs. Known assumptions:

- **Progression order: PCRL (3) and PRCL (5).** PRCL is the ISO/IEC 15444-2 (Part-2) position-resolution level-component-layer order, signalled in COD/COC and (conformantly) advertised via CAP `Ccap^2` bit 14 (`EXTENDED_PROGRESSION`). Any other progression order makes `prepare_precinct_structure` return failure → `create()` returns false → the frame is failed cleanly (counted truncated) rather than silently emitting an empty precinct structure.
- **A DFS marker is required.** `parse_QCD` derives the per-level subband count from `dfs->type[i]`, and `prepare_precinct_structure` derives resolution depths from `dfs->hor_depth/ver_depth`. A plain-dyadic (Mallat) stream with **no** DFS leaves these zero — which under-reads the QCD and historically hung `parse_main_header` (now caught by the bounded scan). The deployment streams are the ATK_DFS_REV class and always carry a DFS marker.
- **Other constraints:** `Csiz == 3` (enforced in `parse_SIZ`), `NL ≤ 5` (`MAX_DWT_LEVEL`), a single quality layer, and **grid-aligned tile origins**. Non-grid-aligned tile origins are invalid per the profile & levels document, so the position loops in `prepare_precinct_structure` step from the tile origin (`coord.{x,y}0 + k·step`); this is identical to the precinct-grid enumeration for every conformant (aligned) origin. A review may flag this as diverging from a general decoder's grid-aligned `x_examin`/`y_examin` enumeration — that divergence only manifests on out-of-profile origins, so **do not "fix" it** (a grid-align change was implemented, validated equivalent to the upstream reference for all origins, then dropped for exactly this reason).
